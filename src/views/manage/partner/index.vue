<template>
  <div class="app-container" style="position: relative;">
    ...
    <el-table v-loading="loading" :data="partnerList" @selection-change="handleSelectionChange">
      ...
      <el-table-column width="70px" label="点位数" align="center" prop="nodeCount" />
      <el-table-column label="分成比例" align="center" prop="profitRatio">
        <template v-slot="scope">
          {{ scope.row.profitRatio }}%
        </template>
      </el-table-column>
      <el-table-column width="70px" label="联系人" align="center" prop="contactPerson" />
      <el-table-column label="联系电话" align="center" prop="contactPhone" />
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="300px">
        <template #default="scope">
            <el-button link type="primary" @click="resetPartnerPwd(scope.row)"
              v-hasPermi="['manage:partner:edit']">重置密码</el-button>
            <el-button link type="primary" @click="handleSelectDetail(scope.row)"
              v-hasPermi="['manage:partner:query']">查看详细</el-button>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="partnerRef" :model="form" :rules="rules" label-width="95px">
        <el-form-item label="合作商名称" prop="partnerName">
    ...
    <!-- 查看合作商详情对话框 -->
    <el-dialog title="合作商详情" v-model="openPartnerInfo" width="500px" append-to-body>
      <el-row>
...
const total = ref(0);
const title = ref("");
const openPartnerInfo = ref(false);
...
/** 重置密码按钮操作 */
function resetPartnerPwd(row) {
  console.log(row);
  const _id = row.id
  proxy.$modal.confirm('是否确认重置名字为"' + row.partnerName + '"的合作商的密码？').then(function () {
    return resetPwd(_id);
  }).then(() => {
    proxy.$modal.msgSuccess("重置密码成功");
  }).catch((err) => { console.log('取消操作', err) });
}

/** 删除按钮操作 */
function handleDelete(row) {
  const _ids = row.id || ids.value;
  proxy.$modal.confirm('是否确认删除名字为"' + row.partnerName + '"的合作商？').then(function () {
